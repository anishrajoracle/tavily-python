...
async def get_company_context():
    async with AsyncTavilyClient(api_key=os.environ["TAVILY_API_KEY"]) as tavily_client:
        return await tavily_client.get_company_info(query=query, search_depth="advanced", max_results=7)


context = asyncio.run(get_company_context())
